...
class _IncomingCallScreenState extends State<IncomingCallScreen> {
  ...
  Future<void> acceptCall(BuildContext context) async {
    ...
    // Do not wait for the token/WebRTC handshake before showing CN CALL's
    // active screen. The manager owns that asynchronous transition.
    unawaited(RtcCallManager.instance.acceptCall(
      callerId: widget.id,
          ...
          child: Column(
            children: [
              ...
              TextButton.icon(
                onPressed: () async {
                  final next = !muted;
                  await RtcCallManager.instance.mute(next);
                  if (mounted) setState(() => muted = next);
                },
                icon: Icon(muted ? Icons.mic_off : Icons.mic),
                label: Text(muted ? 'إلغاء الكتم' : 'كتم'),
              ),
