        ...
        MethodChannel(engine.dartExecutor.binaryMessenger, "cn_call/call").setMethodCallHandler { call, result ->
            when (call.method) {
                ...
                "disconnectTelecomCall" -> CoroutineScope(Dispatchers.Default).launch {
                    ...
                    println("[CN CALL][REJECT DIAGNOSTIC] before disconnectCall call_id=$callId force=$force")
                    val disconnectResult = CoreTelecomCallBridge.disconnectCall(callId, "ended", force)
                    println("[CN CALL][REJECT DIAGNOSTIC] after disconnectCall call_id=$callId boolean=$disconnectResult")
